...
const (
	...
	// MeasuredPodResourcesAnnotationKey is the annotation key used to track which container
	// resources were set by measured pods logic. The value is a comma-separated list of
	// resource types (e.g., "cpu,memory") for the container name.
	MeasuredPodResourcesAnnotationKey = "pod-scaler.openshift.io/measured-resources"
)
...
func (bq *BigQueryClient) Refresh(ctx context.Context) error {
	...
	query := bq.client.Query(fmt.Sprintf(`
		...
		GROUP BY
			org, repo, branch, variant, target, step, pod, container
	...
	for {
		...
		if row.ContainerDurations != "" {
			var durationsMap map[string]string
			if err := json.Unmarshal([]byte(row.ContainerDurations), &durationsMap); err != nil {
				bq.logger.WithError(err).Warnf("Failed to parse container_durations JSON for %s/%s/%s/%s", row.Org, row.Repo, row.Branch, row.Container)
			} else {
				for containerName, durationStr := range durationsMap {
					if duration, err := time.ParseDuration(durationStr); err == nil {
						containerDurations[containerName] = duration
					} else {
						bq.logger.WithError(err).Warnf("Failed to parse duration for container %s", containerName)
					}
				}
			}
		}
...
func AddPodAntiAffinity(pod *corev1.Pod, logger *logrus.Entry) {
	...
	if len(requiredTerms) > 0 {
		// Preserve existing anti-affinity rules by merging instead of overwriting
		...
		if len(existingTerms) > 0 {
			...
			pod.Spec.Affinity.PodAntiAffinity.RequiredDuringSchedulingIgnoredDuringExecution = append(existingTerms, requiredTerms...)
		} else {
			// No existing terms, just set the new ones
