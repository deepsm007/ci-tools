	...
	var testCases = []struct {
		...
		},
		{
			...
			imageMirror: map[string]string{
				...
				"ci/ci:sanitize-prow-jobs":                      "quay-proxy.ci.openshift.org/openshift/ci@sha256:bbb",
			},
			namespace: "ci-op-9bdij1f6",
		},
		{
...
func TestQuayProxyTagFromISKey(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "ci stream",
			isTagKey: "ci/ci:sanitize-prow-jobs",
			wantTag:  "quay-proxy.ci.openshift.org/openshift/ci:ci_ci_sanitize-prow-jobs",
			wantOK:   true,
		},
		{
